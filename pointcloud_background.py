...
def lidar_callback(point_cloud, buf):
    ...
    if background_points is not None:
        tree = cKDTree(background_points)
        d, i = tree.query(points)
        new_points = points[d>1]
        print(new_points.shape)
        if new_points.shape[0] > 0:
            background_points = np.concatenate((background_points, new_points), axis=0)
    else:
        print(points.shape)
    ...
    background_idx += 1
    np.savetxt("background{}.csv".format(background_idx%10), background_points, delimiter=",")
